...
source "[list [file join [file dirname [info script]] ram_phy_ddr_timing.tcl]]"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name board_name ISI_name ram_phy_use_flexible_timing} {
	upvar 1 $t_name t
...
################################################################################
	foreach dqsgroup $pins(dqsgroup) {
		...
		if {$ram_phy_use_flexible_timing} {
			# DQS output clock
			...
			if {$ram_phy_use_flexible_timing && ($::quartus(nameofexecutable) ne "quartus_fit")} {
				lappend ::dqs_clocks $dqs_out_clockname
			}
		}
		# endif $ram_phy_use_flexible_timing
		if {$ram_phy_use_flexible_timing} {
...
set_false_path -to [get_pins -compatibility_mode ${instname}_alt_mem_phy_inst|dpio|dqs_group\[*\].dq\[*\].dqi|auto_generated|input_*_*\[0\]|clrn]
	...
	if {($::quartus(nameofexecutable) == "quartus_fit") || !$ram_phy_use_flexible_timing} {
...
################################################################################
	...
	if {$ram_phy_use_flexible_timing && (($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t board ISI $ram_phy_use_flexible_timing
