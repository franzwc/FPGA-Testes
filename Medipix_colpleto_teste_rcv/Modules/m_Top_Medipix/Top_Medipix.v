...
module Top_Medipix (
      ...
      input                Clk_in_Mdpx,
	   ...
	   wire                 Rst25;
		...
		Medipix_Bridge u_Medipix_Bridge(
		      ...
		      .In_Data_nios                                 (table_nios_data[7:0]),
				...
				.In_En_Equalize                               (w_equalize_valid),
		      .In_Data_Equalize                             (w_equalize_data),
				.In_Send_Equalize										 (w_send_eq),
		...
		Rx_Data_Medipix u_Rx_Data_Medipix (
            ...
            .In_Data_Mdpx                                 (Data_in_Mdpx),
				...
				.In_PS                                        (w_PS),
		      .In_CountL                                    (w_CountL),
				.In_FIFO_Write                                (w_Write_FIFO_Data),
				
				.Out_fifo_use                                 (w_rx_fifo_use),
				.Out_Sync                                     (mdpx_sync),
		...
		Top_Equalizator u_Top_Equalizator (
            ...
            .Out_EQ_En                                    (w_equalize_valid),
				.Out_EQ_Data     									    (w_equalize_data),
				.Out_Send_EQ                                  (w_send_eq)
            );
		//--------------------------------------------------------------------------------------------
		...
		TOP_MEDoIP u_TOP_MEDoIP (
	          .In_Clk_Dado                             	(clk_10),
				 ...
				 .In_ts_PacketLength           	            (Packet_Length+4),
				 .In_En_nios                                 (table_nios_valid),
	          .In_Sync_nios                               (table_nios_sync),
	          .In_Data_nios                               (table_nios_data[7:0]),
				 
				 .Out_fifo_use                               (w_eth_fifo_use),
	   ...
	   Eth_Share u_Eth_Share(
		...
		//--Compartilhamento de ETH
		//--------------------------------------------------------------------------------------------------	
	   Alarm_box u_Alarm_box(
	
				 .In_Reset                                   (Rst25),
	          .In_Clk                                     (clk_10),
	
				 .In_M                        					(w_M),
				 .In_Fifo_Use_Pkt                            (w_rx_fifo_use),
				 .In_Fifo_Use_Eth                            (w_eth_fifo_use),
	
				 .Out_Chip_Busy                              (w_out_busy)
				 );
		//--------------------------------------------------------------------------------------------------
