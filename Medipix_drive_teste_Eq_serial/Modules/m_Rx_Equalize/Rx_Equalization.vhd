...
variable v_contador      : natural range 0 to 100;
variable v_conta_byte    : natural range 0 to 4095;
variable v_conta_pkt     : natural range 0 to 1023;
variable var_rem         : natural range 0 to 1;
variable var_en_fifo     : natural range 0 to 1;
...
begin
	if (In_Rst='1') then
		...
		if ((reg_Port_byte1="0010")and(reg_Port_byte2="0010")and(reg_Port_byte3="1000")and(reg_Port_byte4="1011"))then
			if (v_conta_byte=1)then
				v_conta_pkt := v_conta_pkt+1;
			elsif(v_conta_pkt>384)then
				v_conta_pkt := 1;
			end if;
			
			if ((v_conta_byte>=101) and (v_conta_byte<=2148))then
